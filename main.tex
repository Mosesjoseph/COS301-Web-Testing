...
\section {Missing Functionality - Alpha}
The website runs well and is navigable, though most of the functionality is visible and  you are able to go through different pages and buttons and interact with some of the functionality, most of it has not been fully implemented, due to there being no integration with the back-end functionality.  So to that end most of the functionality cannot be tested, and therefore it is not accurate to say whether certain functionality works or not.  
\newline
...
\paragraph\newline 
This section serves to outline which sections of the Architecture Specification the System partially complies with. This section also serves to outline any shortcomings with the current implementation of the system
...
\section{Short-Comings of the implemented functionality - Bravo}
\paragraph\newline 
There was a large problem with the integration of the back- and front-end of the web service to be provided. The web-interface provides basic functionality to move between the different sections. However, first problem encounter is the login does not work. Pre-conditions to login is that the user should be a register member of the system. The credentials provided need to be checked with the database. The post conditions is that the user should be allowed to pass or forbidden on depending on whether they are registered staff.
\newline\newline
This problem propagates through the entire system because of the disconnect between the back- and front-end of the system. Thus most of the system falls short of what it was intended to do and contracts which where meant to be realised simply fall short due to not being able to utilize the pre-conditions and persist the post-conditions.	
